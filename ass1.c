...
void *processText(char *text)
{
	...
	unsigned char arr[32];//=(unsigned char *)malloc(sizeof(unsigned char)* (textLen));
	...
	for(i=0;i<32;i++)
	{
		...
		if((i+1)%8==0)
		{
			...
			strncat(tempString,myString,10);
			printf("tempString:%s \n",tempString);
			sscanf(tempString,"%x",textArr+j);
			printf("%d: %08x",j,textArr[j]);
			j++;
			myString[0]='\0';
			tempString[0]='\0';
